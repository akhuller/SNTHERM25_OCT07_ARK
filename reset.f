...
c***********************************************************************
      subroutine reset
c
c  Called from converge, ftemp and filtrate
c
...
c
c Passed as  logical array in common l_arrays.txt:
c H2Olayer(nd) .eq. true for Snow,Firn/Ice
...
c      logical H2Olayer
      ...
      do 10 i=1,n - newelm
         ...
         idelete(i)=0
         iskip(i)=0
         call density(to(i),bwo(i),td(i),td13(i),flo(i),blo(i),bi(i),! 7
cREJ_2025/09/17     1        bt(i),dmass(i),bdjp(m),a243(m),bd(m),a1(m),!13
     1        bt(i),dmasso(i),bdjp(m),a243(m),bd(m),a1(m),!13
     2        0,dzo(i),flgo(i),sso(i),dice,ssi(i),porosity(i),dmvol(m) !21
     3        ,ltype(m),impermeable(i),idelete(i), !24
c     3        ,H2Olayer(i),impermeable(i),idelete(i),
