...
c***********************************************************************
      subroutine density(t,bw,td,td13,flo,bl,bi,bt,dmass,bdjp,a243,bd !12
     &     ,a1,melt,dz,flgo,ss,dice,ssi,porosity,dmvol, !21
     &     ltype,impermeable,idelete,solidporosity,ipond, !26
c     &      impermeable,idelete,solidporosity,ipond,
     &      dicevol,dliqvol,rhowater,H2Olayer) !30
c
      implicit none
      include 'const'

c     Note:  These arguments are passing as scalars
c
...
c ltype : Layer type   1=snow 90-95 = Firn/Ice
c                   >99 =user supplied soil layer
c                   2-4 =soil types contained in Block Data file soil
...
c
      integer melt,ltype,impermeable,idelete,ipond,i !REJ 6/24/24
...
c      if(bl .gt. 1d0)then  !REJ3 1/23/24  What is this? Recheck
c      Note on 2/14/24.  C'd out this stop.  Should it be fl .gt. 1d0?
c       write(*,*)'bl,T',bl,T
c       stop 'dens1'
c      endif
CR         stop 'melt in density.f'  !Recheck REJ_2025/09/01 removed for now
         td=0d0
         td13=0d0
         flo=1d0
         flgo=1d0
c        Next handles special case of totally melted snow element.
c REJ 6/24/24   if(ltype .lt. 2 .and. dabs(flo-1d0) .lt. 1d-6 .and. ipond .eq.
         if(H2Olayer .and. dabs(flo-1d0) .lt. 1d-6 .and. ipond .eq.
     &     0)then
c           Setting IDELETE=1 causes this element to combine with
c           its lower neighbor in COMBINENODES.
            t=273.15d0
            idelete=1
c REJ3 1/22/24 +2  IMPORTANT CHANGE           dz=dtol1
            bw = 1d3 ! case where the whole node is water
            dz=dmass/bw 
c REJ3 1/22/24            bw=dmass/dz
            bl=bw
            bi=0d0
            return
         end if
...
c Compute porosity
c
cREJ6/24/24 call porosty(dicevol,dmvol,ltype,porosity,solidporosity)
      call porosty(dicevol,dmvol,H2Olayer,porosity,solidporosity)!REJ6/24/24
c
...
c
cREJ6/24/24      if(ltype .lt. 2)then
      if(H2Olayer)then
        ssim=1d0-ssi
      else 
c Jan29,2002       ssi=bw/(1d3*(1d0-dmvol))+1d-6
        ssim=1d0-ssi  !Recheck.  Why was above abandoned?
      endif
c
