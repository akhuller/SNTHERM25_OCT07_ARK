...
c***********************************************************************
c      double precision function fd(bi) !RBJ 8/1/24. Name conflicted with MatLab
      double precision function fd_init(bi)  
c %W% %G%
      implicit none
      include 'const'
c
...
c  set grain diameter to zero for pure ice or pure liquid water.
      if( dabs(bi - 0.917d3).lt.dtol1 .or. bi .ge. 1.d3)then
         ...
         bisqr=bi*bi
         fd_init=1.6d-4+1.1d-13*bisqr*bisqr
c10/24/95        if(fd .lt. 4d-4)fd=4d-4
         if(fd_init .lt.2.5d-5)fd_init=2.5d-5
      end if
      return
      end
