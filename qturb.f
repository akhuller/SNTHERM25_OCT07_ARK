...
c***********************************************************************
     ...
     2ltype,z0,Cdn,rhoair,bpress,dlogT,dlogQ,dlogW,n,icalcstep) !REJ
c
...
c ltype(ld) :Layer type code (1 = snow, >= 90 = firn, all types ice) !REJ_2025/01/31
...
c
      ...
      integer ltype,n,icalcstep  !REJ_2025/05/21.  Added n and icalcstep
c
c  Local
c 
...
c
       ...
       if (height(i).le.snowdepth)then
         write(*,*)'height(i)=',height(i),'snowdepth=',snowdepth !REJ 1/7/24
         write(*,*) '  **Met Instruments Are Not Above Snow!!**'
         stop '*Error in Layer.in, Line 3. Execution Terminated. Qturb*'!REJ 1/7/24 
       endif
683    continue
cr      write(*,*)
cr     1    'height,Tkair,Tsurface,Eair,Esurface,wsp',
cr     2    'wspo,qlat,qsen,dliqvol,cdryair,Rw,Ck,Csk,snowdepth'
cr      write(*,*)
cr     1     height(3),Tkair,Tsurface,Eair,Esurface,wsp,
cr     2      wspo,qlat,qsen,dliqvol,cdryair,Rw,Ck,Csk,snowdepth,
cr     3     ltype,z0,Cdn,rhoair,bpress,dlogT,dlogQ,dlogW,n
c
...
c      if(ltype .eq. 1 .or. ltype .ge. 90)then !REJ_2025/05/21
      if(H2Olayer(n))then  !REJ_2025/05/21
         Ht=height(1)-snowdepth
         Hw=height(2)-snowdepth
         Hr=height(3)-snowdepth
         dlogW=alog(sngl(Hw/z0))
         sqrtCdn=.4d0/dlogW
CS         if(n.eq.44)write(*,*)'Cdn',sqrtCdn*sqrtCdn,'z0',z0
cS         if(n.eq.44)stop 'qturb'
      else
         Ht=height(1)
         Hw=height(2)
         Hr=height(3)
...
c      if(ltype .eq. 1)
c      if(ltype .eq. 1 .or. ltype .ge. 90)  !REJ_2025/05/21
       if(H2Olayer(n))
...
c       if(ltype .gt. 1 .and. ltype .lt. 90) ! REJ_2025/05/21
        if(H2Olayer(n)) ! REJ_2025/05/21      
     &     freelimv=(dlogT/dlogQ)*freelimv
      endif
c 4.e Compute change in zeta
      y=(Ht/L) - zetat
c      if(it .gt. 40)write(*,*)'Ht/L,zetat,y',Ht/L,zetat,y
