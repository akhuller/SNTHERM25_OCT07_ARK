...
c***********************************************************************
      subroutine old(ido,topflux)!REJ_2025/08/17
c %W% %G%
...
c dmass(nd): CV mass [kg/m^2]  !REJ_2025/06/02
c dmasso(nd): old CV mass [kg/m^2]  !REJ_2025/06/02
...
c
      ...
      do 20 i=1,n
         if(initial .eq. 0)then
            ...
            do(i)=d(i)
            bmelt(i)=0d0
            dmasso(i)=dmass(i) !REJ_2025/06/02
         end if
         if(initial .eq. 1)dmasso(i)=dmass(i) !REJ2025/07/23 Override required for MB_vapor
c          IMPORTANT. I need to check other skipped variables that may need to be included.
         us(i)=0.0
         dsolo(i)=dsol(i)
...
c        unbar(i)=0.0
 ...
 20   continue
      ...
      topfluxo = topflux !REJ_2025/08/17
      return
      end
