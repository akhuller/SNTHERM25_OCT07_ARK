...
c parameters
c REJ note 2025/05/02. Recheck this routine now that sup is no longer
c                      read in and albedo is computed in Sect. 9
c***********************************************************************
c
     ...
     & exp1,ICVPfixd,time0) !REJ_2025/07/24 Add run start time
...
c newsno : time in seconds since start of snowfall. If > 0 signifies 
c          that snowfall has occured recently (within past 48 hours)  REJ2025/06/15.  
...
c time0 : start time of run in hours since the start of year  !REJ:2025/07/24
c Add: passed through common

      integer imetcalc,isolarcalc,islope,iy,jday,ihour,imin
      integer ibasestep,ido,iptype,ircalc,itimezone,ititle
c      integer itm,newsno !REJ_2025/06/15
      integer itm !REJ_2025/06/15
...
c     double precision costheta
      double precision costheta,newsno
...
cNov27,1996      double precision sqrtr, costheta,fvis,albvis,albnir,adjust
      ...
      double precision albvis1,albnir1,fdiffuse,fdate !REJ_2025/05/07
     &    !Added "fdate"
      double precision time0 !REJ_2025/07/24
      logical halbdone,callsol,ICVPfixd,Khuller !  10/22/23  RJ + REJ_2025/01/16
c     logical halbdone,callsol
c*** Functions
c Fd function name conflicted with MatLab variable name      
c REJ 8/1/24 double precision fd,fvapri,fvaprw,dmin1,wetbulb
      double precision fd_init,fvapri,fvaprw,dmin1,wetbulb
      Khuller= .true. !REJ_2025/05/02 check this later.
c 1. OBTAIN MET DATA
      prcp = 0d0 !REJ_2025/08/19.  May be redundant
...
c        Met file in this branch  must contain cloud data
     ...
     &           bwfall,(cover(i),icl(i),i=1,3)
          ...
          if(bifallin .gt. 0d0)then
            read(88,*,end=999)iy,jday,ihour,imin,
     &           tkair,rh,wsp,(rad(i),i=1,3),prcp,iptype,dsnowfall
c REJ_2025/08/20  Moved next 10 lines to precip adjustment block
cc           if(dsnowfall .le. 1d-10)dsnowfall = 1d-4  !REJ_2025/04/30
c            if(dsnowfall .le. 1d-10)dsnowfall = 1d-3  !REJ_2025/06/22 !RECHECK THIS!
cc           Next block just for SUMMIT.  REMOVE LATER
c            if(dsnowfall .le. 1d-10)dsnowfall = 2d-4  !REJ_2025/04/30
c            fdate = dfloat(jday)+dfloat(ihour)/24d0+dfloat(imin)/1440d0      
c            if(prcp .gt. 1d-10)then !REJ_2025/04/30.A small number
c              write(45,'(f15.4,3f15.8)')
c     &        fdate, prcp, prcp*1d6/bifallin,dsnowfall
c     &       !REJ_2025/04/30
c            endif
           else
c Next lines added on Nov 21, 1995
     ...
     &           bwfall
         ...
         if(Khuller)rad(2) = 9999  !  10/22/23  REJ 
      else
         call metcalc(jday,ihour,imin,tkair,rh,wsp,prcp,iptype,
     &        dsnowfall,cover,icl,initial,dtbase,shadow)
      endif
c REJ_2025/07/24  Specify start time of run in hours from start of year. 
c Next 2 lines
      if(ibasestep .eq. 1)
     &  time0 = dfloat((jday-1)*24) + dfloat(ihour) + dfloat(imin)/60d0
...
c        Next uses fixed input albedo option with no slope
        ...
        sup=alb(k(n))*sdown
      END IF
c      sup=sdown*0.65d0  !REJ_2025/01/22  !SUP and ALBEDO: removed on 2025/05/02 
c
c   Compute net solar:
c
c 10/22/23      sup=albsnow*sdown  !TEMPORARY because sup was not being computed
 110  solar=sdown-sup
...
c           REJ_2025/08/19
c     REJ_2025/08/19  Next 3 lines moved from MAIN
c     Boost prcp by x20.  ONLY FOR KM Summit RUN. REMOVE LATER 
c            prcp= 20d0*prcp ! ARK_2025/09/11
            snowrate=prcp/3600.
            if(dabs(prcpo).lt. 1d-15)istart=1 !REJ_2025/08/19

            
c            write(*,*)'GETMET:prcp,snowrate,istart',prcp,snowrate,istart ! ARK_2025/09/16
            rainrate=0d0
c            newsno=max0(1,newsno)  !REJ_2025 redone on 06/15
             newsno = newsno + 1d0 !seconds since fall. updated in
     &          ! subtime.f  REJ_2025/06/15 6/29.  Now use snow_age
...
cREJ_2025/06/28 if(dabs(snowrateo).lt. 1d-15 .or. ltype(k(n)) .gt. 1)
            if(dabs(snowrateo).lt. 1d-15 .or. ltype(k(n)) .ge. 1)
     &         istart=1
            if(prcpo .gt. 0d0)istart = 0d0  ! bandaid to group
     &      ! snowfall in contiguous hours into 1 event.        
...
c After flfall is computed, get tprecip from freezing curve for snow
         ...
         elseif(tkair .gt. 271.15d0)then !REJ_2025/06/07 Bandaid!  Redo later
           tprecip=tkair
           dum=a1snow*(273.15-tprecip)
           flfall=1./(1. + dum*dum)
           if(flfall .lt. 1d-4)flfall=0d0
         else
c          Hold with wetbulb temp computed earlier
           flfall = 0d0 !REJ Recheck this whole block
         endif
         bwfall=dmax1(bwfall,30d0)
         bifall=(1d0-flfall)*bwfall
         blfall=bwfall-bifall
c REJ_2025/08/20.  Next 9 lines moved from read-in block above
c           if(dsnowfall .le. 1d-10)dsnowfall = 1d-4  !REJ_2025/04/30
            if(dsnowfall .le. 1d-10)dsnowfall = 1d-3  !REJ_2025/06/22 !RECHECK THIS!
c           Next block just for SUMMIT.  REMOVE LATER
            if(dsnowfall .le. 1d-10)dsnowfall = 2d-4  !REJ_2025/04/30
            fdate = dfloat(jday)+dfloat(ihour)/24d0+dfloat(imin)/1440d0      
            if(prcp .gt. 1d-10)then !REJ_2025/04/30.A small number
             if(istart .eq. 1)then
              write(45,'(f15.4,2i6,3f15.8,i6)')
     &        fdate,ihour,imin,prcp,prcp*1d6/bifallin,dsnowfall,n+1
              else
              write(45,'(f15.4,2i6,3f15.8,i6)')
     &        fdate,ihour,imin,prcp,prcp*1d6/bifallin,dsnowfall,n
              endif
             endif

         if(dsnowfall.gt. .09 .or.dsnowfall.eq.0) then
c          dsnowfall=fd(bifall) RBJ 8/1/24
           dsnowfall=fd_init(bifall)
         endif
        endif
      else
         snowrate=0d0
...
c Estimated all-band albedo for shallow snow.
