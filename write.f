...
c***********************************************************************
     ...
     &     ssisnow,iqturb,eta0,Khuller,Neumann,Depth,H2Olayer,
     &     snowdepth) ! RJ+RBJ 8/8/02
...
cRJ 12/16/23      double precision height(3),bext,dtmin,dtmax,dtsmin
      ...
      double precision elev,azslope,bifallin,dmlimit,ssisnow,depth(nd)
      double precision snowdepth  !RBJ 8/8/24
...
551   format (/,'WARNING: DID YOU CONVERT PRECIP IN YOUR OLD',
 ...
 30    format (/,20x,'Results of CRREL Snow Temperature Model:REV4',/,/,
     ...
     & /,5x,'Number of Nodes n=',i3)
         ...
         write(80,51)n
 51    format (/,20x,'Results of CRREL Snow Temperature Model',/,
     &  22x,'Jordan and Khuller version 2023',/,/,   !RJ12/17
     & 2x,'Input Data for This Run:',/, 
     & /,5x,'Number of Nodes n=',i3)    
      endif  
...
67      format(5x,'Radiation adjusted for slope of elevation of',f4.0
     ...
     &   ' counterclockwise from North.')
      ENDIF  ! RJ
c RJ 12/16/23      write(80,37)bp,bext,em(k(n))
      write(80,37)bp,frvis,em(k(n))  !RJ 12/16/23
   37 format(5x,'Barometric Pressure (mb) bp=',f6.1/,
     &5x,'Fraction of solar spectrum  0.4 to 1.12 microns=',f6.4/,  !RJ12/17
cRJ 12/14/23     &5x,'Surface Node Extinction Coefficient bext=',f6.2,' (1/m)'/,  !RJ12/17
...
cRJ 12/15/23 73    format('     Rougnhness length and Cd are',f5.2,' (cm) and',f8.5)
73    format('     Rougnhness length and Cd are',f5.2,' (cm) and',f10.5)
      write(80,74)rch(m)
...
C
C  Write 'INITIAL CONDITIONS FROM FILE '
C
          write(80,32) fnm(1)
 32       format(/,10x,'INITIAL CONDITIONS FROM FILE :',a45,//,  !RJ12/17
     ...
     &            to(i),dzo(i),bwo(i),do(i),layertype(i)
          else
             write(80,'(5h     ,i3,5h     ,f6.1,10h          ,f7.3,
...
c         'bmelt' was used as a temporary flag. see comment in main.
 33       continue
          write(80,31)
          do 1 i=2,nfiles
             write(80,'(10h          ,a45)') fnm(i) ! !RJ12/17
 1        continue
 31   format(/,'Other I/O File Names: ',/)  !RJ12/17
      return
      ENDIF
c
...
c Changed from every xth hour to every xth basestep
     ...
     &)/pinv)) .lt. 1d-7 .or. pinv .eq. 99)then
      ...
      write(80,*)
      write(80,'(a19,f8.3)' )'Snow/Icedepth(m) ',snowdepth

c 12/22/23 RJ 268 write(80,61)(i,phase(i),dz(i),z(i)-soildepth,t(i),bt(i),bwo(i),                   
       write(80,61)(i,phase(i),dz(i),z(n)+(dz(n)/2)-z(i),t(i),bt(i),!RJ12/17
     &    bwo(i),blo(i),ct(i),thk(i),d(i),layertype(i),i=n,1,-1)  
c 10/22/23 RJ 61   format(/,2x,'i',1x,'P',5x,'dz(i) ',6x,'z(i) ',6x,'t(i) ',7x,
 61   format(/,2x,'i',1x,'P',5x,'dz(i) ',4x,'  -Z(i) ',4x,'t(i) ',7x, !RBJ
     & 'bt(i) ',6x,'bw(i) ',6x,'bl(i) ',6x,'ct(i) ',4x,'thk(i) ',4x,
     & 'd(i)  ',1x, 'layer',/,
     & 11x,'(m)',8x,'(m)',8x,'(K)',8x,'(kg/m^3)',4x,'(kg/m^3)',4x,
     & '(kg/m^3)',3x,'(J/kg-K)',3x,'(W/m-K)',4x,' (m)  '
c 10/22/23  RJ     & //,102(i3,1x,a1,3x,f8.5,3x,f8.5,4x,f8.3,3(2x,f10.4),3x,
     & //,102(i3,1x,a1,3x,f8.5,3x,f9.4,4x,f8.3,3(2x,f10.4),3x,
     & f8.1,3x,f8.5,1x,f8.6,a10,/)/)
      do 62 m=1,n
        i = n+1-m
        depth(i) =z(n)+(dz(n)/2)-z(i)
62    continue
c
C     TEMPERATURE PROFILES FOR SNOW
c
      do_profiles = .false.  !RJ 12/15/23
      If(do_profiles)then    !RJ 12/15/23
      profile(1:1)='o'
...
C
C     PRINT OUT RESULTS FOR THIS ITERATION
C
 ...
 152  format(//,2x,'date-time',9x,'t(n)',7x,'t(sg)'
     &     ,6x,'tkair',4x,1x,'nodes',1x, 'calc-iter',/)
      if(nsoil .ge. 1 .and. n .gt. nsoil)then !REJ3 1/27/24
         tsg= (t(nsoil)*dz(nsoil+1)+t(nsoil+1)*dz(nsoil))/
     &     (dz(nsoil+1)+dz(nsoil))
      elseif(nsoil .eq. 0)then !REJ3 1/27/24
         tsg = T(1) !REJ3 1/27/24
      else
         tsg=t(nsoil)
      endif
      if(itm.ge.1) then
 ...
 52   format(i2,i3,1x,i3,i3,2x,5f11.3,3x,f7.3,5x,f7.3,2x,i4,
     &     1x,a1,i7,1x,a1)
 53   format(i2,i3,1x,i3,i3,2x,3f11.3,2x,i4,1x,a1,i12,1x,a1)  !Khuller
c 10/22/23  RJ 53   format(i2,i3,1x,i3,i3,2x,3f11.3,2x,i4,1x,a1,i7,1x,a1)
