...
c Rwm : gas constant for water vapor (8.3145) [j/moles-K]
...
c j = layer index 
c ii = CV (control volume) or nodal index of total stack
c i = CV or nodal index within each layer in stack
c k(ii) = j
c M = 

c Passed by arrays. !REJ: Incomplete list
c e0 : vapor saturation at 1000mb T = 273.15d0 (= 6.1368 mb) !REJ_2025
c   Later change 273.15 to 273.16                             /07/21
...
c  Next assigns nodes/CVs(ii) to a layer type(j)
c  k(ii) stores layer subscript j corresponding to node ii
c  Loop through all elements ii within each layer j
c  nn(j) is the number of nodes/Cvs in each layer
c  As usual, all numbers from the bottom up
       do 33 i=1,nn(j)
c         layer type choices are snow = 1, soil = 2 to 4, ice/firn = 90 to 95
c         See GETINPUT.f, where the types codes are read from the layer.in file
          ii=ii+1
          k(ii)=j
          if(ltype(j) .ge. 2 .and. ltype(j) .le. 4)then 
c         SOIL
            layertype(ii)=soiltype(ltype(j)) !sand,silt,clay
...
c           dummy value.
c 10/22/23             do(ii) = 1d-4
          elseif(ltype(j) .ge. 90 .and.ltype(j) .le. 95)then
c         FIRN/ICE
c            Nothing to compute here yet
          elseif(ltype(j) .eq. 1)then
c         SNOW
             ssi(ii)=ssisnow
