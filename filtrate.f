...
cc***********************************************************************
      subroutine filtrate(ib,it,*,dzmin,Neumann)!RJ 12/10/23 Added Neumann
c %W% %G%
c
c Called from MAIN 
c
...
c        write(*,*)'i,s(i)',i,sso(i) !REJ_2025/01/21
         sso(i) = sso(i) - 1d-6
c        stop 'Filtrate:Saturated flow not allowed in sntherm.89' !REJ3
      end if
c
...
30    continue
c
      If(Neumann)then !Start change REJ 1/17/24
        call tridiagN(n2,a,x,b,Neumann)!REJ 1/17/24
      else
        call tridiag(n2,a,x,b,Neumann) !RJ 12/10/23  Added Neumann
      endif !REJ End changed 1/17/24
c
